...
{
	// Abre o arquivo INI e recupera o diretorio para os arquivos de log
	CInitFile oDirLog;
	...
	if( sz <= 0 )
	{
		...
		long tam = strlen(ARQ);
		tam -= 14;
		strcpy(&ARQ[tam],oDt.DT);
		strcat(ARQ,".LOG"); // acrescenta a extensao .LOG ao arquivo
	}
	else
	{
		strcat(ARQ,"\\");
		strcat(ARQ,oDt.DT); // diretorio recuperado do arquivo ini
		strcat(ARQ,".LOG"); // acrescenta a extensao .LOG ao arquivo
	}
...
VOID CLogFile::SetLogInterno(char *ponto,char *msg)
{
	...
	AppendText("%s ; %s ; %s ; %s", oDt.DT, MSGINTERNAS, ponto, msg);
}